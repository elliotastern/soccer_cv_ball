...
<head>
    ...
    <title>Ball Validation - {Path(annotation_file).name}</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        ...
        <p>Dataset: {Path(annotation_file).name}</p>
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python 9_validate_100_frames_template.py <annotation_file>")
        sys.exit(1)
