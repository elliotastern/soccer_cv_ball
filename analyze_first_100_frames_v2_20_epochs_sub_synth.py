#!/usr/bin/env python3
"""
Analyze first 100 frames using trained ball detection model (20 epochs, SoccerSynth_sub)
Same structure as v1_out_of_box but uses the trained model instead of out-of-box RF-DETR
"""
import cv2
import numpy as np
...
def detect_with_trained_model(frame: np.ndarray, model=None) -> list:
    ...
    ball_detections = detect_balls(model, frame, confidence_threshold=0.3)
...
def analyze_frames(video_path: str, num_frames: int = 100):
    ...
    """
    print(f"📊 Analyzing first {num_frames} frames with TRAINED MODEL (20 epochs, SoccerSynth_sub)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("📈 DETECTION METRICS (First 100 Frames) - TRAINED MODEL")
    print("=" * 60)
