#!/usr/bin/env python3
"""
Analyze first 100 frames using trained ball detection model (35 epochs, SoccerSynth_sub + Validation images OFFICIAL)
Same structure as v2_20_epochs_sub_synth_plus_sahi but WITHOUT SAHI - uses standard detection
"""
import cv2
import numpy as np
...
def detect_with_trained_model(frame: np.ndarray, model=None) -> list:
    """
...
def analyze_frames(video_path: str, num_frames: int = 100):
    ...
    """
    print(f"📊 Analyzing frames 500-599 with TRAINED MODEL + SAHI (35 epochs, SoccerSynth_sub + Validation images OFFICIAL)")
    print("=" * 60)
    ...
    print("✅ Model loaded")
    print("✅ SAHI enabled: Slice size=1288, Overlap=20% (higher definition processing)\n")
    ...
    print(f"Processing frames {start_frame}-{start_frame+num_frames-1} with SAHI (higher definition)...")
    ...
    for i in range(num_frames):
        ...
        detections = detect_with_trained_model(frame, model=trained_model)
        ...
        # Track objects (temporarily disabled due to tracker issue)
        try:
            ...
            # Tracker error - continue without tracking for now
            tracked_objects = []
            tracked_players = []
            tracked_balls = []
    ...
    start_frame = 500
    end_frame = start_frame + len(frame_metrics) - 1
    print("\n" + "=" * 60)
    print(f"📈 DETECTION METRICS (Frames {start_frame}-{end_frame}) - TRAINED MODEL + SAHI (35 epochs, SoccerSynth_sub + Validation images OFFICIAL)")
    print("=" * 60)
    ...
    print(f"\n⚽ BALLS (with SAHI - higher definition):")
    print(f"  Detections per frame:")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Analyze frames with trained model (35 epochs, SoccerSynth_sub + Validation images OFFICIAL)')
    parser.add_argument('--num-frames', type=int, default=100, help='Number of frames to analyze (default: 100)')
    args = parser.parse_args()
    
    video_path = "data/raw/real_data/F9D97C58-4877-4905-9A9F-6590FCC758FF.mp4"  # 4K video (2880x3840)
    analyze_frames(video_path, num_frames=args.num_frames)
