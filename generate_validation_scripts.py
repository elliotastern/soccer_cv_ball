#!/usr/bin/env python3
"""
Generate individual validation scripts for each annotation file.
"""
from pathlib import Path

# Template for individual validation scripts
...
ANNOTATION_FILES = [
    '/workspace/soccer_coach_cv/models/ball_detection/dataset/train/_annotations.coco.json',
    '/workspace/soccer_coach_cv/models/ball_detection/dataset/valid/_annotations.coco.json',
    '/workspace/soccer_coach_cv/models/ball_detection/dataset/test/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/combined_ball_detection/train/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/combined_ball_detection/valid/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/combined_ball_detection/train_mosaic/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/combined_ball_detection/valid_mosaic/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/SoccerSynth_sub/train_coco/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/SoccerSynth_sub/test_coco/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/SoccerSynth_sub/train_coco_mosaic/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/SoccerSynth_sub/test_coco_mosaic/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/real_data/Validation images OFFICIAL/train/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/real_data/Validation images OFFICIAL/valid/_annotations.coco.json',
    '/workspace/soccer_coach_cv/data/raw/real_data/Validation images OFFICIAL/test/_annotations.coco.json',
]
...
def generate_scripts():
    ...
    for ann_file in ANNOTATION_FILES:
        ...
        if not ann_path.exists():
            print(f"⚠️  Skipping {ann_file} (not found)")
            continue
        
        # Generate script filename based on path
        # Get parent directory name and create unique identifier
        parent_dir = ann_path.parent.name
        grandparent_dir = ann_path.parent.parent.name if ann_path.parent.parent.name else ""
        
        # Create unique filename
        if grandparent_dir and grandparent_dir != 'soccer_coach_cv':
            filename_base = f"{grandparent_dir}_{parent_dir}"
        else:
            filename_base = parent_dir
        
        # Clean up filename
        filename_base = filename_base.replace(' ', '_').replace('-', '_').replace('(', '').replace(')', '')
        script_name = f"9_validate_100_frames_{filename_base}.py"
        
        script_path = Path("/workspace/soccer_coach_cv") / script_name
        
        # Fill in the template
        script_content = SCRIPT_TEMPLATE.replace("{ANNOTATION_FILE}", ann_file)
    ...
    print(f"\n✅ Generated {generated} validation scripts")
