#!/usr/bin/env python3
"""
Visualize first 100 frames with trained ball detection model (20 epochs, SoccerSynth_sub)
...
def detect_with_trained_model(frame: np.ndarray, model=None, threshold: float = 0.3) -> list:
    """
    Use trained ball detection model (20 epochs on SoccerSynth_sub)
    ...
    # Detect balls using trained model
    ball_detections = detect_balls(model, frame, confidence_threshold=threshold)
...
def main():
    ...
    threshold = 0.3
    num_frames = 100
    output_dir = Path("data/output/visualizations_v2_20_epochs_sub_synth")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"📊 Visualizing First {num_frames} Frames with TRAINED MODEL (20 epochs, SoccerSynth_sub)")
    print("=" * 60)
    ...
    html_path = output_dir / "summary_v2_20_epochs_sub_synth.htm"
...
<html>
<head>
    <title>Trained Model Detection Results - 20 Epochs SoccerSynth_sub</title>
    <style>
...
<body>
    <div class="header">
        <h1>Detection Results - Trained Model (20 Epochs, SoccerSynth_sub)</h1>
