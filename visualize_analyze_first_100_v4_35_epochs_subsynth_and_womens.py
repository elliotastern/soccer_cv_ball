#!/usr/bin/env python3
"""
Visualize first 100 frames with trained ball detection model (35 epochs, SoccerSynth_sub + Validation images OFFICIAL)
Standard detection (no SAHI)
"""
import cv2
import numpy as np
...
def detect_with_trained_model(frame: np.ndarray, model=None, threshold: float = 0.3) -> list:
    """
    Use trained ball detection model (35 epochs on SoccerSynth_sub + Validation images OFFICIAL)
    ...
    # Detect balls using trained model (standard detection, no SAHI)
...
def main():
    video_path = "data/raw/real_data/F9D97C58-4877-4905-9A9F-6590FCC758FF.mp4"  # 4K video (2880x3840)
    threshold = 0.3
    num_frames = 100
    output_dir = Path("data/output/visualizations_v4_35_epochs_subsynth_and_womens")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"📊 Visualizing First {num_frames} Frames with TRAINED MODEL (35 epochs, SoccerSynth_sub + Validation images OFFICIAL)")
    print("=" * 60)
    ...
    print("✅ Model loaded")
    print("✅ Standard detection (no SAHI)\n")
    ...
    print(f"Processing and visualizing frames {start_frame}-{start_frame+num_frames-1} with SAHI (higher definition)...")
    ...
    for i in range(num_frames):
        ...
        detections = detect_with_trained_model(frame, model=trained_model, threshold=threshold)
        ...
        if (i + 1) % 20 == 0:
            print(f"  Processed frame {frame_num} ({i + 1}/{num_frames})...")
    ...
    html_path = output_dir / "summary_v4_35_epochs_subsynth_and_womens.html"
...
<html>
<head>
    <title>Trained Model Detection Results - 35 Epochs SoccerSynth_sub + Validation images OFFICIAL</title>
    <style>
...
<body>
    <div class="header">
        <h1>Detection Results - Trained Model (35 Epochs, SoccerSynth_sub + Validation images OFFICIAL)</h1>
        <p>Model: Ball-only detection trained on SoccerSynth_sub + Validation images OFFICIAL (combined dataset)</p>
        <p>Standard detection (no SAHI)</p>
    ...
    <div class="note">
        ...
        It does not detect players. Model trained for 35 epochs on combined dataset: SoccerSynth_sub (synthetic) + Validation images OFFICIAL (real data).
        <br><strong>SAHI Enabled:</strong> Slicing Aided Hyper Inference improves detection by processing image tiles at 1288x1288 resolution for higher definition processing.
    </div>
